...
def final_model(input_dim, filters, kernel_size, conv_stride,
    ...
    layer = Bidirectional(cell(units, activation=activation,
            return_sequences=True, implementation=2, name='rnn_1', dropout=dropout_rate), merge_mode='concat')(conv_bn)
    layer = BatchNormalization(name='bt_rnn_1')(layer)
    if number_of_layers > 1:
        for i in range(number_of_layers - 2):
            layer = Bidirectional(cell(units, activation=activation,
                        return_sequences=True, implementation=2, name='rnn_{}'.format(i+2), dropout=dropout_rate), merge_mode='concat')(layer)
            layer = BatchNormalization(name='bt_rnn_{}'.format(i+2))(layer)

        layer = Bidirectional(cell(units, activation=activation,
                    return_sequences=True, implementation=2, name='final_layer_of_rnn'), merge_mode='concat')(layer)
